...
class PurchasesController extends BaseController
{
    ...
    public function index(request $request)
    {
        ...
        $Purchases = Purchase::with('facture', 'provider', 'store')
        ...
        foreach ($Purchases as $Purchase) {
            ...
            $item['store_name'] = $Purchase['store']->name;
        ...
        $accounts = Account::where('deleted_at', '=', null)->orderBy('id', 'desc')->get(['id','account_name']);
         ...
         //get stores assigned to user
         $user_auth = auth()->user();
         if($user_auth->is_all_stores){
             $stores = store::where('deleted_at', '=', null)->get(['id', 'name']);
         }else{
             $stores_id = Userstore::where('user_id', $user_auth->id)->pluck('store_id')->toArray();
             $stores = store::where('deleted_at', '=', null)->whereIn('id', $stores_id)->get(['id', 'name']);
         } 
            ...
            foreach ($data as $key => $value) {
                ...
                if ($order->statut == "received") {
                    if ($value['product_variant_id'] !== null) {
                        $product_store = product_store::where('deleted_at', '=', null)
                            ->where('store_id', $order->store_id)
                            ->where('product_id', $value['product_id'])
                            ->where('product_variant_id', $value['product_variant_id'])
                            ->first();

                        if ($unit && $product_store) {
                            if ($unit->operator == '/') {
                                $product_store->qte += $value['quantity'] / $unit->operator_value;
                            } else {
                                $product_store->qte += $value['quantity'] * $unit->operator_value;
                            }
                            $product_store->save();
                ...
                foreach ($old_purchase_details as $key => $value) {
                    ...
                    if($value['purchase_unit_id'] !== null){
                        if ($current_Purchase->statut == "received") {
                            ...
                            if ($value['product_variant_id'] !== null) {
                                $product_store = product_store::where('deleted_at', '=', null)
                                    ->where('store_id', $current_Purchase->store_id)
                                    ->where('product_id', $value['product_id'])
                                    ->where('product_variant_id', $value['product_variant_id'])
                                    ->first();
                    ...
                    if($prod_detail['no_unit'] !== 0){
                        ...
                        if ($request['statut'] == "received") {
                            ...
                            if ($prod_detail['product_variant_id'] !== null) {
                                ...
                                if ($unit_prod && $product_store) {
                                    if ($unit_prod->operator == '/') {
                                        $product_store->qte += $prod_detail['quantity'] / $unit_prod->operator_value;
                                    } else {
                                        $product_store->qte += $prod_detail['quantity'] * $unit_prod->operator_value;
                                    }
    ...
    public function show(Request $request, $id)
    {
        ...
        $purchase_data['store'] = $purchase['store']->name;
    ...
    public function edit(Request $request, $id)
    {
        if (PurchaseReturn::where('purchase_id', $id)->where('deleted_at', '=', null)->exists()) {
            ...
            if ($Purchase_data->store_id) {
                if (store::where('id', $Purchase_data->store_id)->where('deleted_at', '=', null)->first()) {
                    $purchase['store_id'] = $Purchase_data->store_id;
                } else {
                    $purchase['store_id'] = '';
                }
            } else {
            ...
            foreach ($Purchase_data['details'] as $detail) {
                ...
                if ($detail->product_variant_id) {
                    $item_product = product_store::where('product_id', $detail->product_id)
                        ->where('deleted_at', '=', null)
                        ->where('product_variant_id', $detail->product_variant_id)
                        ->where('store_id', $Purchase_data->store_id)
                        ->first();
                    ...
                    $item_product = product_store::where('product_id', $detail->product_id)
                        ...
                        ->where('store_id', $Purchase_data->store_id)->first();
    ...
    public function get_Products_by_purchase(Request $request , $id)
    {
        ...
        $Return_detail['store_id'] = $Purchase_data->store_id;
                ...
                \DB::transaction(function () use ($request, $data) {
                    ...
                    foreach ($data as $key => $value) {
                        ...
                        if ($order->statut == "received") {
                           ...
                           if ($unit && $product_store) {
                               if ($unit->operator == '/') {
                                   $product_store->qte += $value['qty'] / $unit->operator_value;
                               } else {
                                   $product_store->qte += $value['qty'] * $unit->operator_value;
                               }
                               $product_store->save();
